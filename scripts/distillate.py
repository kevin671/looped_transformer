...
def main(args, device):
    ...
    optimizer = torch.optim.Adam(
        student_model.parameters(), lr=args.training.learning_rate, weight_decay=args.training.weight_decay)
    ...
    for i in pbar:
        ...
        loss, output, total_norm, grad_norm_dict = train_step(args, args.training.curriculum.loops.end, teacher_model, student_model, xs, ys, optimizer, ctx, scaler)

        # EVALUATION ======================================
        point_wise_tags = list(range(args.training.curriculum.points.end))  # [0, 1, 2, ..., n-1]
